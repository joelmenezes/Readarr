...
export const actionHandlers = handleThunks({
  ...
  [DELETE_BOOK_FILE]: function(getState, payload, dispatch) {
    const {
      id: bookFileId,
      albumEntity: bookEntity = bookEntities.BOOKS
    } = payload;
  ...
  [DELETE_BOOK_FILES]: function(getState, payload, dispatch) {
    ...
    promise.done(() => {
      ...
      const tracksWithRemovedFiles = bookFileIds.reduce((acc, bookFileId) => {
        acc.push(..._.filter(tracks, { bookFileId }));
